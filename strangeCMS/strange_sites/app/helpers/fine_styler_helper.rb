...
module FineStylerHelper
    ...
    group = options[:group]
      siza = Strangecms::Stylez::Config["#{area}_#{pref}_icon_size"].to_s
	    stuff = '<div class="toggle_list_inna_trigger">'
  		    ...
  		    stuff += "</h3></div>"
  		    stuff += "<div class='fl_box_310' id='#{area}_#{pref}_vorschau'>#{ render "admin/settings/fl_#{ thing }", :pref => pref, :area => area, :group => group  }</div>"
  		    stuff += "<div class='fl_box_110 zeus'><div class='settings_callback #{ pref }'> </div></div>"
  ...
  def ff_font_styler( pref, options = {} )
     options.assert_valid_keys(:area, :hover, :active, :thing, :group)
     options.reverse_merge! :area    =>    'front'    unless    options.key? :area
     options.reverse_merge! :hover    =>    true      unless    options.key? :hover
     options.reverse_merge! :active   =>    true      unless    options.key? :active
     ...
     group = options[:group]
       siza = Strangecms::Stylez::Config["#{area}_#{pref}_icon_size"].to_s
 	     stuff = '<div class="toggle_list_inna_trigger">'
   		    ...
   		    stuff += "</h3></div>"
   		    stuff += "<div class='fl_box_230'><div class='vorschau' style='#{ fine_font_style( pref, area ) }'>"
